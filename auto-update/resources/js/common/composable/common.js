...
const common = () => {
    ...
    const isSubdomainModuleEnabled = computed(() => store.state.auth.isSubdomainModuleEnabled);
    ...
    const getRootDomain = () => {
        ...
        if (parts.length <= 2) {
            return host; // already root domain like example.com or localhost
        }

        // Handles domains like sub.example.com or sub.sub2.example.co.in
        return parts.slice(-2).join("."); // returns example.com or co.in
    }


    const generateFullUrl = (subdomain) => {
