    ...
    public function globalSetting()
    {
        ...
        if (app_type() == 'saas' && $module && $module->isEnabled()) {
            ...
            if ((($subdomain == null && $mainApplicationUrl == '') || $subdomain == $mainApplicationUrlSubdomain) === false) {
                $settings = Company::where('subdomain', $subdomain)->first();
            } else {
    ...
    public function superAdminLogin(LoginRequest $request)
    {
        ...
        if ($user) {
            ...
            if (app_type() == 'saas' && $module && $module->isEnabled()) {
                ...
                if ((($subdomain == null && $mainApplicationUrl == '') || $subdomain == $mainApplicationUrlSubdomain) === false) {
                    $subdomainCompany = Company::where('subdomain', $subdomain)->first();
                } else {
                    $subdomainCompany = GlobalCompany::first();
                }

                $credentials['company_id'] = $subdomainCompany->id;
            }
        }
