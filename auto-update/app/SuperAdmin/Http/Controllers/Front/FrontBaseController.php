...
class FrontBaseController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            ...
            $this->globalCompany = GlobalCompany::first();

            $this->isSubdomainModuleInstalled = Module::has('StockiflySaasSubdomain') && Module::isEnabled('StockiflySaasSubdomain') && app_type() == 'saas';

            $host = request()->getHost(); // e.g., sub.example.com
            $parts = explode('.', $host);

            if (count($parts) >= 2) {
                $this->domainName = implode('.', array_slice($parts, -2)); // example.com
            } else {
                $this->domainName = $host; // fallback
            }
