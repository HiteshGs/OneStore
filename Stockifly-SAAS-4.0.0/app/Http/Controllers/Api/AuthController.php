...
class AuthController extends ApiBaseController
{
    ...
    public function getVerfiedEmailSetting(EmailIndexRequest $request)
    {
        $verified = false;
    ...
    public function sendMail($xid, $lang = "en")
    {
        ...
        if (!$customer || !$customer->email || !filter_var($customer->email, FILTER_VALIDATE_EMAIL)) {
            throw new ApiException('Customer email is not valid or missing.');
        }

        $pdfObject = $this->getPdfObject($order->unique_id, $lang);

        $subject = 'Invoice for Order #' . $order->id;
        $message = 'Please find the invoice with invoice number ' . $order['invoice_number'] . ' attached.';

        Notification::route('mail', $customer->email)
            ->notify(new InvoiceMail($subject, $message, base64_encode($pdfObject['pdf']->output())));
    ...
    public function getPdfObject($uniqueId, $langKey = "en")
    {
        $order = Order::with(['warehouse', 'user', 'items', 'items.product', 'items.unit', 'orderPayments:id,order_id,payment_id,amount', 'orderPayments.payment:id,payment_mode_id', 'orderPayments.payment.paymentMode:id,name'])
            ->where('unique_id', $uniqueId)
            ->first();
    ...
    public function pdf($uniqueId, $langKey = "en")
    {
        $pdfObject = $this->getPdfObject($uniqueId, $langKey);
        $order = $pdfObject['order'];

        // $pdf = app('dompdf.wrapper');
        // $pdf->loadHTML($html);
        //    return $pdf->stream('document.pdf');
        return $pdfObject['pdf']->download($order->invoice_number . '.pdf');
    }
