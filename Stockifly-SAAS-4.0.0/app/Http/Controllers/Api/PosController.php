...
class PosController extends ApiBaseController
{
    ...
    public function getOrderForInvoice($xid)
    {
        $orderId = $this->getIdFromHash($xid);
        ...
        ->with([
            'user:id,name,email,phone,address,city,state,gst_number',
            ...
            'staffMember:id,name',
            'warehouse:id,name,address,phone,email,city,state,gst_number,bank_account_no,bank_name,bank_branch,ifsc_code,invoice_terms,show_phone_on_invoice,show_email_on_invoice,logo'
        ])
        ->find($orderId);
        ...
        foreach ($order->items as $item) {
            // Try to get product data (including soft deleted)
            $product = Product::withTrashed()->find($item->product_id);
            
            if ($product) {
                // Create a product object with necessary data
                $item->product = (object)[
                    'id' => $product->id,
                    'xid' => $product->xid,
                    'name' => $product->name,
                ...
                // Get product details for current stock
                ...
                if ($productDetails) {
                    $item->product->details = (object)[
                        'id' => $productDetails->id,
                        'product_id' => $product->id,
                        'warehouse_id' => $order->warehouse_id,
                        'current_stock' => $productDetails->current_stock,
                        'xid' => \Vinkla\Hashids\Facades\Hashids::encode($productDetails->id),
                        'x_product_id' => $product->xid,
                        'x_warehouse_id' => \Vinkla\Hashids\Facades\Hashids::encode($order->warehouse_id),
                    ];
                }
            } else {
                // Product not found even in trash - create minimal object
                $item->product = null;
            }
            ...
            $customFields = [];
            if ($item->product_id) {
                $productCustomFields = \DB::table('product_custom_fields')
                    ->join('custom_fields', 'custom_fields.id', '=', 'product_custom_fields.custom_field_id')
                    ->where('product_custom_fields.product_id', $item->product_id)
                    ->select('custom_fields.name as label', 'product_custom_fields.value')
                    ->get();
                
                foreach ($productCustomFields as $cf) {
                    $customFields[] = [
                        'label' => $cf->label,
                        'value' => $cf->value
                    ];
        ...
        $savingOnMrp = $totalMrp - $order->total;
        $saving_percentage = $totalMrp > 0 ? number_format((float)($savingOnMrp / $totalMrp * 100), 2, '.', '') : 0;

        $order->saving_on_mrp = $savingOnMrp;
        $order->saving_percentage = $saving_percentage;
        $order->total_tax_on_items = $totalTax + $order->tax_amount;

        // Convert amount to words
        $order->amount_in_words = $this->convertNumberToWords($order->total);

        // Add warehouse logo URL
        if ($order->warehouse && $order->warehouse->logo) {
            $order->warehouse->logo_url = url('storage/' . $order->warehouse->logo);
        }

        // Get entry person name from staff member
        if ($order->staffMember) {
            $order->entry_person_name = $order->staffMember->name;
        }

        return ApiResponse::make('Order fetched successfully', [
            'order' => $order,
        ]);
    }
