    ...
    <div id="pos-invoice">
      ...
      >
        ...
        <div class="company-details">
          ...
          <h4 style="margin-bottom: 0px">{{ $t('common.phone') }}: {{ selectedWarehouse.phone }}</h4>
        ...
        <div class="tax-invoice-details">
          ...
          <table class="invoice-customer-details">
            <tbody>
              <tr>
                <td style="width:50%">{{ $t('sales.invoice') }} &nbsp;&nbsp;&nbsp;&nbsp;: {{ order.invoice_number }}</td>
                <td style="width:50%">{{ $t('common.date') }} : {{ formatDate(order.order_date) }}</td>
              </tr>
              <tr>
                <td style="width:50%">{{ $t('stock.customer') }} : {{ order.user.name }}</td>
                <td style="width:50%">{{ $t('stock.sold_by') }} : {{ order.staff_member.name }}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table style="width:100%">
            <thead style="background:#eee">
              <tr>
                ...
                <td :style="{ width: selectedWarehouse.show_mrp_on_invoice ? '10%' : '25%' }">
                  {{ $t('common.qty') }}
                </td>
                <td v-if="selectedWarehouse.show_mrp_on_invoice" style="width:20%">{{ $t('product.mrp') }}</td>
                <td :style="{ width: selectedWarehouse.show_mrp_on_invoice ? '20%' : '25%' }">{{ $t('common.rate') }}</td>
                <td :style="{ width: selectedWarehouse.show_mrp_on_invoice ? '20%' : '25%', textAlign: 'right' }">
                  {{ $t('common.total') }}
                </td>
              </tr>
            </thead>
            <tbody>
              <tr class="item-row" v-for="(item, index) in order.items" :key="item.xid">
                ...
                <td v-if="selectedWarehouse.show_mrp_on_invoice">{{ item.mrp ? formatAmountCurrency(item.mrp) : '-' }}</td>
                <td>{{ formatAmountCurrency(item.unit_price) }}</td>
                <td style="text-align:right">{{ formatAmountCurrency(item.subtotal) }}</td>
              </tr>

              <tr class="item-row-other">
                <td :colspan="selectedWarehouse.show_mrp_on_invoice ? 4 : 3" style="text-align:right">
                  {{ $t('stock.order_tax') }}
                </td>
                <td colspan="2" style="text-align:right">{{ formatAmountCurrency(order.tax_amount) }}</td>
              </tr>
              ...
              <tr class="item-row-other">
                ...
                <td colspan="2" style="text-align:right">{{ formatAmountCurrency(order.discount) }}</td>
              </tr>
              ...
              <tr class="item-row-other">
                ...
                <td colspan="2" style="text-align:right">{{ formatAmountCurrency(order.shipping) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="tax-invoice-totals">
          <table style="width:100%">
            <tbody>
              <tr>
                <td style="width:30%"><h3 style="margin-bottom:0px">{{ $t('common.items') }}: {{ order.total_items }}</h3></td>
                <td style="width:30%"><h3 style="margin-bottom:0px">{{ $t('common.qty') }}: {{ order.total_quantity }}</h3></td>
                <td style="width:40%; text-align:center"><h3 style="margin-bottom:0px">{{ $t('common.total') }}: {{ formatAmountCurrency(order.total) }}</h3></td>
              </tr>
            </tbody>
          </table>
        </div>
              ...
              <tr style="text-align:center">
                <td style="width:100%">
                  <h4 style="margin-bottom:0px" v-if="order.order_payments">
                    ...
                    <span v-for="p in order.order_payments" :key="p.xid" style="margin-right:5px">
                      {{ formatAmountCurrency(p.amount) }}
                      (<span v-if="p.payment?.payment_mode?.name">{{ p.payment.payment_mode.name }}</span>)
                    </span>
                  </h4>
                  <h3 style="margin-bottom:0px" v-else>{{ $t('invoice.payment_mode') }}: -</h3>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div v-if="selectedWarehouse.show_discount_tax_on_invoice" class="discount-details">
          <p>{{ $t('invoice.total_discount_on_mrp') }} : {{ formatAmountCurrency(order.saving_on_mrp) }}</p>
          <p>{{ $t('invoice.total_discount') }} : {{ order.saving_percentage }}%</p>
          <p>{{ $t('invoice.total_tax') }} : {{ formatAmountCurrency(order.total_tax_on_items) }}</p>
        </div>

        <div v-if="selectedWarehouse.barcode_type == 'barcode'" class="barcode-details">
          <BarcodeGenerator :value="order.invoice_number + ''" :height="25" :width="1" :fontSize="15" :elementTag="'svg'" />
        </div>
...
export default defineComponent({
  ...
  components: { PrinterOutlined, BarcodeGenerator, QRcodeGenerator, SendOutlined },
  setup(props, { emit }) {
    const { t } = useI18n();
...
.company-details { text-align: center; margin-top: 5px; border-bottom: 2px dotted #ddd !important; }
.company-address { white-space: break-spaces; margin-bottom: 0px; }
.invoice-customer-details { width: 100%; margin-bottom: 5px; }
.tax-invoice-title { text-align: center; margin-top: 5px; }
.tax-invoice-items { margin-top: 10px; }
.item-row { border-bottom: 1px dotted #ddd !important; }
.tax-invoice-totals { margin-top: 5px; border-top: 2px dotted #ddd !important; border-bottom: 2px dotted #ddd !important; }
...
.thanks-details { margin-top: 5px; text-align: center; }
.barcode-details { margin-top: 10px; text-align: center; }
.footer-button { text-align: center !important; }
.discount-details { padding: 5px 0px; border-top: 2px dotted #ddd !important; border-bottom: 2px dotted #ddd !important; }
.discount-details p { margin-bottom: 0px; }

/* size previews */
.invoice-root { width: 100%; }
.a4-invoice { max-width: 210mm; }
.a5-invoice { max-width: 148mm; }
.thermal-80 { max-width: 80mm; }
.thermal-58 { max-width: 58mm; }
.thermal-80, .thermal-58 { font-size: 12px; }
</style>
