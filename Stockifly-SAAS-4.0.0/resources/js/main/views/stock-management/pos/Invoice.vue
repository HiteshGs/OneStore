<template>
  ...
  >
    ...
    <span>Generated by: {{ getUserDisplayName() }}</span> | 
...
const getUserDisplayName = () => {
  ...
  return authUser?.display_name || 'Guest'; // Default to 'Guest' if no name is found
};

const currentDateTime = computed(() => {
  const now = new Date();
  return now.toLocaleString('en-IN', { hour12: true }); // Format the date and time as per your requirement
});
