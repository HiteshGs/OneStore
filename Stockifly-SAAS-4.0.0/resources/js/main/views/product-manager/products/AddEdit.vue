...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const generateBarCode = () => {
            const selectedBarcodeSymbology = props.formData.barcode_symbology;

            if (selectedBarcodeSymbology == "EAN8") {
                props.formData.item_code = generateEAN8();
            } else if (selectedBarcodeSymbology == "EAN13") {
                props.formData.item_code = generateEAN13();
            } else if (selectedBarcodeSymbology == "UPC") {
                props.formData.item_code = generateUPC();
            } else {
                props.formData.item_code = parseInt(Math.random() * 1000000000);
            }
        };

        const generateEAN8 = () => {
            // EAN-8 requires 7 digits and the last digit is a checksum.
            const base = getRandomDigits(7);
            return base + calculateEAN8Checksum(base);
        };

        const generateEAN13 = () => {
            // EAN-13 requires 12 digits and the last digit is a checksum.
            const base = getRandomDigits(12);
            return base + calculateEAN13Checksum(base);
        };

        const generateUPC = () => {
            // UPC requires 11 digits and the last digit is a checksum.
            const base = getRandomDigits(11);
            return base + calculateUPCChecksum(base);
        };
        ...
        const calculateEAN8Checksum = (code) => {
            let sum = 0;
            for (let i = 0; i < code.length; i++) {
                let num = parseInt(code[i]);
                if (i % 2 === 0) {
                    // EAN checksum rules: multiply odd-positioned digits (0, 2, 4, ...) by 3
                    sum += num * 3;
                } else {
                    // Even-positioned digits (1, 3, 5, ...) are added directly
                    sum += num;
                }
            }
            // Calculate the checksum digit as (10 - (sum mod 10)) % 10
            const checksum = (10 - (sum % 10)) % 10;
            return checksum;
        };

        const calculateUPCChecksum = (code) => {
            let sum = 0;
            for (let i = 0; i < code.length; i++) {
                let num = parseInt(code[i]);
                if (i % 2 === 0) {
