                ...
                <div class="table-responsive">
                    ...
                    >
                        <template #bodyCell="{ column, record }">
                            <template v-if="column.dataIndex === 'status' && record && record.details">
                            ...
                            <template v-if="column.dataIndex === 'sales_price'">
                                <span v-if="(productType == 'single' || productType == 'service') && record.details">
                            ...
                            <template v-if="column.dataIndex === 'purchase_price'">
                                <span v-if="productType == 'single' && record.details">
                            ...
                            <template v-if="column.dataIndex === 'current_stock'">
                                <a-typography-link v-if="productType == 'single' && record.details" type="primary" style="margin-left:4px" @click="openProductDetails(record)">
                                    {{ `${record.details.current_stock} ${record.unit.short_name}` }}
                                </a-typography-link>
                                <a-typography-link v-else-if="productType == 'variable'" type="primary" style="margin-left:4px" @click="openProductDetails(record)">
                            ...
                            <template v-if="column.dataIndex === 'action'">
                                <a-space>
                                    ...
                                    <a-button v-if="permsArray.includes('products_edit') || permsArray.includes('admin')" type="primary" @click="editItem(record)">
                                        <template #icon><EditOutlined /></template>
                                    </a-button>
                                    <a-button v-if="permsArray.includes('products_delete') || permsArray.includes('admin')" type="primary" @click="showDeleteConfirm(record.xid)">
                                        <template #icon><DeleteOutlined /></template>
                                    </a-button>
                                </a-space>
                            </template>
                        ...
                        <template v-if="productType == 'variable'" #expandedRowRender="productData">
                            ...
                            >
                                <template #bodyCell="{ column, record }">
                                    ...
                                    <template v-if="column.dataIndex === 'sales_price' && record.details">{{ formatAmountCurrency(record.details.sales_price) }}</template>
                                    <template v-if="column.dataIndex === 'purchase_price' && record.details">{{ formatAmountCurrency(record.details.purchase_price) }}</template>
                                    <template v-if="column.dataIndex === 'current_stock' && record.details">{{ `${record.details.current_stock} ${productData.record.unit.short_name}` }}</template>
                                    <template v-if="column.dataIndex === 'action'">
                                        <a-button @click="viewItem(record)" type="links"><template #icon><EyeOutlined /></template></a-button>
                                    </template>
                                </template>
                            </a-table>
                        </template>
...
import { EyeOutlined, PlusOutlined, EditOutlined, DeleteOutlined, ClusterOutlined, AppstoreAddOutlined } from "@ant-design/icons-vue";
...
export default {
    components: { EyeOutlined, PlusOutlined, EditOutlined, DeleteOutlined, ClusterOutlined, AppstoreAddOutlined, AddEdit, ProductView, ProductHistory, AdminPageHeader, ImportProducts, SubscriptionModuleVisibility, SubscriptionModuleVisibilityMessage },
    setup() {
        const { t } = useI18n();
        ...
        const loadWarehouses = async () => {
            try { const { data } = await axiosAdmin.get("warehouses?fields=id,xid,name&per_page=200"); warehouses.value = [{ xid:null,name:"All Warehouses" }, ...data]; } catch(e) { warehouses.value = [{ xid:null,name:"All Warehouses" }]; }
        };
        onMounted(() => { loadWarehouses(); getInitialData(); setUrlData(); crudVariables.crudUrl.value = addEditUrl; crudVariables.langKey.value = "product"; crudVariables.initData.value = {...initData}; crudVariables.formData.value = {...initData}; crudVariables.hashableColumns.value = [...hashableColumns]; crudVariables.multiDimensalObjectColumns.value = {...multiDimensalObjectColumns}; });
        ...
        const totals = computed(()=>{
            ...
            crudVariables.table.data.forEach(tr=>{
                if(productType.value=='variable' && tr.variations) totalCurrentStock+=getVariableProductStockSum(tr);
                else if(productType.value=='single' && tr.details?.current_stock) totalCurrentStock+=tr.details.current_stock;
            });
        ...
        return {
            ...
            categories, brands, filters, warehouses, filterTreeNode, setUrlData, sampleFileUrl, productType, filteredProducts,
            getVariableProductSalePrice, getVariableProductPurchasePrice, getVariableProductStockSum, totals,
            productDetailsVisible, modelData, openProductDetails, onCloseProductDetails
        };
    }
};
</script>
