...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // ---------- local error helpers (mirror apiAdmin behavior) ----------
    const formatDetails = (details) => {
      if (!details || typeof details !== 'object') return [];
      const lines = [];
      Object.keys(details).forEach((k) => {
        const arr = Array.isArray(details[k]) ? details[k] : [details[k]];
        arr.forEach((m) => lines.push(`${k}: ${String(m)}`));
      });
      return lines;
    };
    ...
    const extractServerError = (err) => {
      ...
      return { topMsg, lines: formatDetails(details) };
    };
    // -------------------------------------------------------------------
      ...
      try {
        ...
        while (true) {
          const { data } = await axiosAdmin.get('warehouses', {
            params: { per_page: perPage, page, fields: 'xid,name', order: 'name' },
          });
        ...
        const { topMsg, lines } = extractServerError(e);
        const full = [topMsg, ...lines].join('\n');
        message.error(full); // show exact backend message
        // eslint-disable-next-line no-console
        console.error('Failed to load warehouses:', e?.response?.data || e);
        allWarehouses.value = [];
      } finally {
