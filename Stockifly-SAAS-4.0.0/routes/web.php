...
// (Optional) You don't need the import below because routes use 'uses' => '...@method' strings.
...
ApiRoute::group(['namespace' => 'App\Http\Controllers\Api'], function () {
    ...
    // ✅ NEW: PUBLIC endpoint for parent-warehouse dropdown
    ApiRoute::get('warehouses/options', [
        'as'   => 'api.warehouses.options',
        'uses' => 'WarehouseController@options',
    ]);
    ...
    ApiRoute::group(['middleware' => ['api.auth.check']], function () {
        ...
        ApiRoute::group(['middleware' => ['api.permission.check', 'api.auth.check', 'license-expire']], function () {
            ...
            ApiRoute::post('pos/invoice-items', ['as' => 'api.pos.invoice-items', 'uses' => 'PosController@invoiceItems']);
            ApiRoute::get('pos/staff-members', ['as' => 'api.pos.staff-members', 'uses' => 'PosController@getStaffMembers']);
            ...
            // Update warehouse online_store_enabled
...
}); // ✅ close: Admin API namespace group
...
ApiRoute::group(['namespace' => 'App\Http\Controllers\Api\Hrm'], function () {
    ...
    ApiRoute::group(['middleware' => ['api.auth.check', 'license-expire']], function () {
        ...
        // Leaves will be visible to everyone (controller-level permissions apply elsewhere)
